...
def plot_arena_grid(ax,G_or_L,line_color=None,line_style='--',show_grid_lines=True,grid_alpha=0.8,cmap_grid=None):
    ...
    G_or_L -> either just L (int) == the size of the square grid side
              or G: the grid matrix generated by the mouse walk
    """
    ...
    if show_grid_lines:
        ...
        l_y = numpy.linspace(Y_arena_lim[0],Y_arena_lim[1],L+1)[1:-1]
        for x in l_x:
            l.append(ax.plot([x,x],Y_arena_lim,color=line_color,linestyle=line_style,linewidth=1))
        for y in l_y:
            l.append(ax.plot(X_arena_lim,[y,y],color=line_color,linestyle=line_style,linewidth=1))
    if type(G_or_L) is numpy.ndarray:
        l.append(ax.imshow(G_or_L,extent=(X_arena_lim[0],X_arena_lim[1],Y_arena_lim[1],Y_arena_lim[0]),alpha=grid_alpha,cmap=cmap_grid ) )
    if show_grid_lines:
        rect_wh = abs(X_arena_lim[1] - X_arena_lim[0])
        rect_xy = (X_arena_lim[0],Y_arena_lim[1]) # we use the index 1 in Y because Y is switched
        grid_contour = plt.Rectangle( rect_xy, rect_wh, rect_wh, edgecolor=line_color, facecolor='none', linestyle=line_style, linewidth=1, zorder=100)
...
def plot_arena_sketch(track,showAllEntrances=False,arenaPicture=False,ax=None,showHoles=True,showArenaCircle=True,holesColor=None,showStart=False,startArgs=None,bgCircleArgs=None,holesArgs=None):
    """
    plots arena sketch
    
    arenaPicture -> it is either bool (and if so, this function tries to load the arena picture that corresponds to track
                    from the predetermined mousetrack/arena_picture directory)
    ...
    if has_arena_pic_to_plot(arenaPicture):
        if ax is None:
            _,ax = plt.subplots(figsize=(fig_w,fig_h))
            ax.set_position([ (1.0-ax_w)/2.0, (1.0-ax_h)/2.0, ax_w, ax_h ])
        if type(arenaPicture) is bool:
            arena_pic = plib.get_cropped_arena_picture(track,bgcolor_rgba=(1,1,1,1))
        else:
            arena_pic = arenaPicture
        if len(arena_pic) > 0:
            ax.imshow(arena_pic, extent=[track.arena_pic_left, track.arena_pic_right, track.arena_pic_bottom, track.arena_pic_top]) 
    else:
        if ax is None:
            _,ax = plt.subplots(figsize=(fig_w,fig_h))
            ax.set_aspect('equal')
            ax.set_position([ (1.0-ax_w)/2.0, (1.0-ax_h)/2.0, ax_w, ax_h ])
        ax.set_xlim([track.arena_pic_left,track.arena_pic_right])
        ax.set_ylim([track.arena_pic_bottom,track.arena_pic_top])
        ax.set_autoscale_on(False)
    if showArenaCircle:
        arena_circ = plt.Circle(track.r_arena_center, track.arena_diameter/2.0, **_get_kwargs(bgCircleArgs,edgecolor='k',fill=False))
        ax.add_patch(arena_circ)
    if showAllEntrances:
        kk = 0
        for lab,c in plib.get_arena_entrance_coord(track).items():
            valign,halign = _get_text_anchor_alignment(c)
            ax.annotate(lab,c,verticalalignment=valign, horizontalalignment=halign)
            ax.plot(c[0],c[1],'x',color=(0.8,0.0,0.8,1.0),markersize=6,zorder=1000+kk)
            kk += 1
    if showStart:
        txt,txtArgs = _get_label(startArgs) # must come before so that the 'pop' removes labelArgs before the plot
        if misc.exists(txt):
            padding = txtArgs.pop('pad',numpy.zeros(2))
            ax.annotate(txt,numpy.asarray(track.r_start)+padding,**txtArgs)
        ax.plot(track.r_start[0],track.r_start[1],**_get_kwargs(startArgs,marker='s',markeredgewidth=3,color='k',fillstyle='none'))
    if showHoles:
        if type(holesColor) is type(None):
            holesColor = (0.8,0.8,0.8,1)
        ax.plot(track.r_arena_holes[:,0],track.r_arena_holes[:,1],'o',**_get_kwargs(holesArgs,markersize=3,fillstyle='none',linewidth=0.1,color=holesColor))
    return ax
...
def plot_all_tracks_2targets(track_complete,hole_horizon,time_delay_after_food,traj1Args=None,traj2Args=None,
                             ...
                             show_title=True,title_args=None,nrows_ncols_tuple=None,show_probetitle=True,
                             fix_mouse_order_in_probe=True,show_speed=False):
    start_align_vector = start_align_vector if misc.exists(start_align_vector) else (-1,1)
    fig_size           = fig_size           if misc.exists(fig_size) else (18,10)
    traj1Args          = traj1Args if misc.exists(traj1Args) else dict()
    traj2Args          = traj2Args if misc.exists(traj2Args) else dict()
    ...
    track_complete_rot = plib.rotate_trial_file(copy.deepcopy(track_complete),start_align_vector,True)

    if trim_trajectories:
        track12 =   tran.keep_path_between_targets(track_complete_rot,return_t_in_targets=False,    hole_horizon=hole_horizon,time_delay_after_food=time_delay_after_food,copy_tracks=True)
        track1  = [ tran.remove_path_after_food(tr,tr12.r_nose[0,:],False,force_main_target=True,hole_horizon=hole_horizon,time_delay_after_food=time_delay_after_food,copy_tracks=True) for tr,tr12 in zip(track_complete_rot,track12) ]
    else:
        track1  = track_complete_rot
        track12 = [None for _ in range(len(track_complete_rot))]

    #panel_ind = dict(zip([33,36,57,58,35,34,59,60],range(len(track_complete_rot)))) # indices are chosen to match Kelly's slides
    fix_panel_ind_condition = lambda tr: (tr.trial == 'Probe2') and (tr.exper_date in [ '22Jun2021', '19Nov2021' ])
    panel_ind               = dict(zip([33,35,36,57,59,58,60,34],range(len(track_complete_rot)))) # indices are chosen to match Kelly's slides
    if not misc.exists(axes_to_plot):
    ...
    #for i in range(2):
    #    for j in range(4):
    for k in range(len(track_complete_rot)):
        tr     = track_complete_rot[k]          #tr  = track[1][k]
        tr1    = track1[k]         #trf = track_filtered[1][k]
        tr12   = track12[k]
        ax_ind = k
        if fix_mouse_order_in_probe:
            ax_ind = panel_ind[int(tr.mouse_number)] if fix_panel_ind_condition(tr) else k

        i,j = numpy.unravel_index(ax_ind,nrows_ncols_tuple)
        if show_speed:
            i = 2*i # arena row index

        ax[i,j] = plot_arena_sketch(  tr, ax=ax[i,j],arenaPicture=False, showHoles=show_arena_holes, showArenaCircle=True)
        l.append(plot_mouse_trajectory(ax[i,j],tr1,  **traj1Args))
        if misc.exists(tr12):
            l.append(plot_mouse_trajectory(ax[i,j],tr12, **traj2Args))
        ax[i,j].set_aspect('equal','box')
        if len(probe_title) > 0:
            txt = probe_title
        else:
            txt = 'trial %s'%(tr.trial)
        if show_probetitle:
            title_txt = 'mouse %s, %s'%(tr.mouse_number,txt)
        else:
            title_txt = 'mouse %s'%(tr.mouse_number)
        if show_title:
            ax[i,j].set_title(title_txt,**title_args)
        ax[i,j].axis('off')
    result = ax.reshape(nrows_ncols_tuple),l
    if return_panel_ind:
        result += (panel_ind,)
    return result
...
def _plot_hole_check_spatial_distribution(track,r,r_count,r_target,r_mean,r_eigdir,r_disp,
                                          r_target2     = None,
                                          ax            = None,
